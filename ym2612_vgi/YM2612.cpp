...
bool isVgi(char* filename) {
  int8_t len = strlen(filename);
  return strstr(strlwr(filename + (len - 4)), ".vgi");
}

static bool sd_initialized = false;

void YM2612::loadPatch (){
  File root;
  VGI_t inst;
  ...
  while (!found) {
    ...
    if (isVgi(entry.name())) {
      if(i==patch_index){
        found = true;
        if (entry.read(&inst,sizeof(inst))) {


          setUnison(true);
          setAMS(0);//inst.AMD_FMD
          setFMS(0);
          setFeedback(inst.Feedback);
          setAlgorithm(inst.Algorithm);
          
          for(int i=0;i<4;i++){

            setAttackRate(-1,i,inst.ops[i].Attack_Rate);
            setDecayRate(-1,i,inst.ops[i].Decay_Rate_AM_enable);
            setSustainRate(-1,i,inst.ops[i].Sustain_Rate);
            setReleaseRate(-1,i,inst.ops[i].Release_Rate);
            setTotalLevel(-1,i,inst.ops[i].Total_Level);
            setSustainLevel(-1,i,inst.ops[i].Sustain_Level);
            setMultiply(-1,i,inst.ops[i].Multiplier);
            setDetune(-1,i,inst.ops[i].Detune);
            setRateScaling(-1,i,inst.ops[i].Rate_Scaling);
            //setAmplitudeModulation(-1,i,inst.ops[i].Attack_Rate);
            setSSG_EG(-1,i,inst.ops[i].SSG_EG);
          }
        }
      }
      i++;
...
void YM2612::sendData(uint8_t data) {
  ...
  // take the latchPin low so data don't change while you're sending in bits:
...
void YM2612::setChannelParameter(int chan, int reg_offset, int val_size, int val_shift, int val) {
  ...
  for (int c = 0; c < 6; c++){
    if ( (unison && chan==-1 ) ||  (selected_channel==c && chan==-1 ) ||  (chan==c) ) {
      int channel_part = c / 3;
      uint8_t* p = (uint8_t *) ( &channels[channel_part]) + reg_offset;
      int channel_offset = c % 3; //which of the 12 registers
...
void YM2612::setOperatorParameter(int chan,int oper, int reg_offset, int val_size, int val_shift, int val) {
  for (int c = 0; c < 6; c++){
    if ( (unison && chan==-1 ) ||  (selected_channel==c && chan==-1 ) ||  (chan==c) ) {
      for (int o = 0; o < 4; o++){
        if ( (oper==-1 && operators[o] ) || (oper==o) ) {
          int channel_part = c / 3;
          uint8_t* p = (uint8_t *) ( &channels[channel_part]) + reg_offset;
          int op_offset = c % 3 + o * 4; //which of the 12 registers
